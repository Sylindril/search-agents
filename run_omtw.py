...
OMTW VERSION: Supports raw images with coordinate-based actions.
"""
import argparse
import collections
import copy
...
from browser_env.actions import (
    is_equivalent, create_goto_url_action,
    create_mouse_click_action, create_mouse_hover_action,
    create_scroll_action, create_key_press_action,
    create_keyboard_type_action, create_none_action
)
...
# =====================================================
# COORDINATE-BASED ACTION PARSING FOR RAW IMAGE MODE
# =====================================================
# Expected format from LLM:
#   click [x, y]
#   type [x, y] [text to type]
#   scroll [up|down]
#   press [key_combo]
#   stop [answer]
#   goto [url]

def create_coord_based_action(response: str) -> Action:
    """Parse coordinate-based action from LLM response."""
    ...
    for pattern, action_type in patterns:
        ...
        if match:
            if action_type == 'click':
                ...
                action = create_mouse_click_action(x, y)
                ...
                action = {
                    ...
                    "element_role": 0,
                    "element_name": "",
                    "pw_code": f"page.click({{x: {x}, y: {y}}}).then(() => page.keyboard.type('{text}'))",
                ...
                x, y = int(match.group(1)), int(match.group(2))
                action = create_mouse_hover_action(x, y)
            
            action["raw_prediction"] = response
            return action
    
    # If no pattern matched, return NONE action
    action = create_none_action()
    action["raw_prediction"] = response
    return action


def save_debug_screenshot(img_array, save_path: str) -> str:
    """Save screenshot for debugging and return path."""
    img = Image.fromarray(img_array)
    img.save(save_path)
    return save_path


def print_debug_info(step: int, intent: str, url: str, prompt: str, response: str, 
                     action: Action, screenshot_path: str = None):
    ...
    print(response)
    print("-"*40 + " PARSED ACTION " + "-"*40)
    print(f"Action Type: {action.get('action_type', 'N/A')}")
    print(f"Raw Prediction: {action.get('raw_prediction', 'N/A')}")
    if 'coords' in action:
        print(f"Coordinates: {action['coords']}")
    if 'text' in action and action.get('action_type') == ActionTypes.TYPE:
...
def config() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--action_set_tag", default="id_accessibility_tree", 
        help="Action type: id_accessibility_tree, som, coord (for raw image mode)"
    )
    parser.add_argument(
        "--observation_type",
        choices=[
        ...
        help="Observation type (use 'image' for raw screenshots without SoM)",
    )
    parser.add_argument(
        "--debug", action="store_true", 
        help="Enable debug output (print prompts, responses, save screenshots)"
    )
    parser.add_argument(
    ...
    if args.action_set_tag == "coord":
        # Coordinate-based actions work with raw image observation
        if args.observation_type != "image":
            print(f"WARNING: coord action_set_tag is designed for 'image' observation type, got {args.observation_type}")
    elif (
...
def test(
    ...
    for config_file in config_file_list:
        ...
        try:
            ...
            debug_dir = None
            if args.debug:
                debug_dir = Path(args.result_dir) / "debug" / str(task_id)
            ...
            # Save initial screenshot if debug mode
            if args.debug and debug_dir:
                init_screenshot_path = debug_dir / "step_0_initial.png"
                Image.fromarray(obs["image"]).save(init_screenshot_path)
                print(f"[Step 0] Initial screenshot saved: {init_screenshot_path}")
                print(f"[Step 0] URL: {env.page.url}")
            ...
            while True:
                ...
                if args.agent_type == "search" and type(action) == list:
                    ...
                    if args.debug:
                        print(f"\n{'='*60}")
                        print(f"[Step {step_idx}] SEARCH: Generated {len(action)} candidate actions:")
                        for i, a in enumerate(action):
                            print(f"  [{i}] {a.get('raw_prediction', str(a))[:100]}...")
                        print(f"{'='*60}")
                    ...
                    def take_action_and_score(a, curr_trajectory, curr_action_history,
                        ...
                        if args.debug:
                            print(f"\n  [SEARCH] depth={depth}, a_idx={a_idx}: Executing action...")
                            print(f"    Action: {a.get('raw_prediction', str(a))[:80]}...")
                        ...
                        # Save debug screenshot during search
                        if args.debug and debug_dir:
                            search_img_path = debug_dir / f"search_step{step_idx}_d{depth}_a{a_idx}.png"
                            Image.fromarray(obs["image"]).save(search_img_path)
                            print(f"    Screenshot: {search_img_path}")
                ...
                for best_idx, action in enumerate(best_actions):
                    ...
                    # Debug output for action
                    if args.debug:
                        print(f"\n{'='*60}")
                        print(f"[Step {step_idx}] ACTION TO EXECUTE:")
                        print(f"  Type: {action.get('action_type', 'N/A')}")
                        print(f"  Raw: {action.get('raw_prediction', 'N/A')}")
                        if 'coords' in action:
                            print(f"  Coords: {action['coords']}")
                        if 'text' in action and action.get('action_type') == ActionTypes.TYPE:
                            print(f"  Text: {action['text']}")
                        print(f"{'='*60}")
                    ...
                    if action["action_type"] == ActionTypes.STOP:
                        if args.debug:
                            print(f"[Step {step_idx}] STOP action - task complete")
                    ...
                    # Debug output after action
                    if args.debug and debug_dir:
                        screenshot_path = debug_dir / f"step_{step_idx}_after.png"
                        Image.fromarray(obs["image"]).save(screenshot_path)
                        print(f"[Step {step_idx}] Screenshot saved: {screenshot_path}")
                        print(f"[Step {step_idx}] URL after: {env.page.url}")
