...
IMPORTANT: Only give full reward (mark as "success") when the task is FULLY COMPLETE, not when it appears to be almost complete or about to be complete. If the agent is close to completion but hasn't finished, mark it as "on the right track" but still a "failure". The agent must issue a stop action with the correct output to be considered successful. If there is no stop action, it is considered a failure.
